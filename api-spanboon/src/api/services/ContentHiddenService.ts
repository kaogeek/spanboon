...
import { ContentHidden } from '../models/ContentHidden';
import { ContentHiddenRepository } from '../repositories/ContentHiddenRepository';
import { SearchUtil } from '../../utils/SearchUtil';

@Service()
export class ContentHiddenService {

    constructor(@OrmRepository() private contentHiddenRepository: ContentHiddenRepository) { }

    // find ContentHidden
    public async find(findCondition: any): Promise<ContentHidden[]> {
        return this.contentHiddenRepository.find(findCondition);
    }

    // find ContentHidden
    public findOne(findCondition: any): Promise<ContentHidden> {
        return this.contentHiddenRepository.findOne(findCondition);
    }

    // create ContentHidden
    public async create(ContentHidden: ContentHidden): Promise<ContentHidden> {
        return await this.contentHiddenRepository.save(ContentHidden);
    }

    // update ContentHidden
    public async update(query: any, newValue: any): Promise<any> {
        return await this.contentHiddenRepository.updateOne(query, newValue);
    }

    // delete ContentHidden
    public async delete(query: any, options?: any): Promise<any> {
        return await this.contentHiddenRepository.deleteOne(query, options);
    }

    // aggregate ContentHidden
    public aggregate(query: any, options?: any): Promise<any[]> {
        return this.contentHiddenRepository.aggregate(query, options).toArray();
    }

    // Search ContentHidden
